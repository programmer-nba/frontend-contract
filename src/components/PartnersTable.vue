...
<template>
    <div class="card">
        <DataTable class="p-5" showGridlines :value="partners" tableStyle="min-width: 50rem">
            <template #header>
            ...
            <Column class="border p-2 text-center" header="ชื่อบริษัท">
                <template #body="partner">
                    <div class="flex gap-2 items-center">
                        <span>{{ partner.data.partner_company_name }}</span>
                    </div>
                </template>
            </Column>
            <Column class="border p-2" header="เบอร์โทรบริษัท">
                <template #body="partner">
                   {{ partner.data.partner_company_phone }}
                </template>
            </Column>
            <Column class="border p-2" header="สถานะ">
                <template #body="partner">
                   <span class="px-2 rounded-full" :class="(partner.data.status_appover==='รออนุมัติ' ? 'bg-orange-500' : '')"></span>{{ partner.data.status_appover }}
                </template>
            </Column>
            <Column class="border p-2" header="ร่างสัญญา">
                <template #body="partner">
                    <router-link :to="{ name: 'ContractId', params: { id: `${partner.data._id}` }}">
                        <Button class="p-2 rounded bg-orange-500 text-white" label="ร่างสัญญา" />
                    </router-link>
                </template>
            </Column>
            
            <template #footer> จำนวนพาร์ทเนอร์ทั้งหมด {{ partners ? partners.length : 0 }}</template>
        </DataTable>
    </div>
</template>
